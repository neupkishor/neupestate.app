import { getAuthAccountToken, setAuthAccountToken, subscribeAuthAccountToken } from '#/core/auth/session';
...
export function AuthSessionProvider({ children }: PropsWithChildren) {
  ...
  useEffect(() => subscribeAuthAccountToken(setTokenState), []);

  useEffect(() => {
    void getAuthAccountToken()
      ...
      try {
        ...
        if (canRefresh && status === 401) {
          ...
          if (refreshedToken) {
            ...
            await setAuthAccountToken(refreshedToken);
        ...
        setExpired(true);
        setTokenState(null);
        await setAuthAccountToken(null);
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    const handleUrl = ({ url }: { url: string }) => {
      ...
      if (returnedToken) {
        ...
        void setAuthAccountToken(returnedToken);
      }
    };
  ...
  const setToken = async (nextToken: string | null) => {
    ...
    if (nextToken) await setAuthAccountToken(nextToken);
    else await setAuthAccountToken(null);
  };
